...
| `leagues` / `league_participants` / `league_matches` | **League (round-robin) format (v2.56.0)** — mirror of `brackets`. `leagues` (1:1 with a tournament, `status`, points 3/1/0, tiebreaker order), `league_participants` (seeded roster), `league_matches` (round-robin calendar with scores + `is_draw`, reusing the `match_state` enum). Selected when `tournaments.competition_format='league'`. Standings are derived on read (`src/lib/league/standings.ts`), never stored |
