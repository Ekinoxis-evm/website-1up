...
// attested/approved by an admin (no comprobante). `admin_grant` is admin-only,
// never user-selectable.
export type PassPaymentMethod = "token" | "bank" | "cash";

// Per-service toggle from service_payment_methods (service = 'pass'). Only
// `cash_enabled` gates anything user-facing here — token + bank are always on
// (the historical default). Default = today's live behavior (cash off) so callers
...
// per-service toggle on. Any other value (including admin_grant) is rejected —
// admin grants come through the admin POST, never the user path.
