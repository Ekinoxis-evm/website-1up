...
export function WalletTab({ cashEnabled = false, cardEnabled = false }: { cashEnabled?: boolean; cardEnabled?: boolean }) {
  ...
  // Bottom tab
  const [tabView, setTabView] = useState<"historial" | "ordenes">("historial");
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex">
        <button
          onClick={() => setTabView("historial")}
          className={`flex-1 py-3 font-headline font-black text-xs uppercase tracking-widest transition-colors ${
            tabView === "historial"
        ...
        <div className="bg-surface-container-low">
          {txLoading ? (
            ...
            <div className="px-6 py-12 flex flex-col items-center justify-center gap-3 text-center">
              ...
              <p className="text-xs font-headline uppercase tracking-widest text-on-surface/30">Sin transacciones</p>
              <p className="text-xs font-body text-on-surface/20 max-w-[200px]">Aquí aparecerá tu historial de $1UP</p>
            </div>
          ) : (
            <>
              ...
              <div className="flex flex-col">
                {pagedTxItems.map((tx) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-3 px-6 py-3.5 hover:bg-surface-container transition-colors group border-b border-outline-variant/10"
                  >
                    <div className={`w-8 h-8 flex items-center justify-center shrink-0 ${
                      tx.direction === "send" ? "bg-primary-container/15 text-primary-container" : "bg-secondary-container/15 text-secondary"
                    }`}>
