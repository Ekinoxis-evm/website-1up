...
| `treasury_wallets` | **v2.48.0** — admin-managed on-chain destination wallets for $1UP: label, address (EVM, CHECK), chain_id (default 8453/Base), is_active, sort_order. RLS deny-all (service-role only). Managed on the **Cuentas y Tesorerías** page (Sistema group); tournaments pick a wallet from this list (the chosen `address` is stored into `tournaments.treasury_address`, so on-chain verification is unchanged). Endpoint `GET\|POST\|PUT\|DELETE /api/admin/treasury-wallets` |
| `token_purchase_orders` | user_profile_id FK, privy_user_id, email, nombre, celular_contacto, wallet_address, cop_amount, token_amount, exchange_rate_cop (frozen 1000), payment_method (token/bank/cash/**card** — card v2.52.0; card records the COP receipt then leaves the order `pending` for the admin's on-chain $1UP send), bank_account_id FK, comprobante_url, status (pending/approved/rejected/cancelled), admin_notes, rejection_reason, approved_tx_hash, reviewed_by, reviewed_at |
...
| `tournaments` | name, slug (unique — auto-generated from name, used in URLs), game_id FK (nullable → games — category badge derived from `games.category_id`, v2.51.0), date, prize_pool_cop (deprecated — use tournament_prizes), max_participants, status (upcoming/live/completed), location_type (presencial/online/mixto), image_url, description, sponsor_name, sponsor_website_url, sponsor_logo_url, **sponsor_logo_bg** (transparent/white/black — display background for the sponsor logo; URL-or-PNG-upload, rounded display; v2.51.0), is_active, is_registration_open, sort_order, **bank_account_id** FK → bank_accounts (nullable — per-tournament account that receives wire/cash entry fees; the body's `bankAccountId` is the fallback; v2.51.0), **entry_fee_tokens** (NUMERIC nullable), **entry_fee_cop** (INT nullable) — both null = free tournament (v2.41.0), **treasury_address** (text nullable, EVM address — per-tournament $1UP treasury; required when entry_fee_tokens > 0, never reuses pass_config; v2.41.0) |
| `tournament_entry_orders` | Entry-fee payment record (v2.41.0, mirrors pass_orders; **RLS-on/deny-all since v2.43.0**) — tournament_id FK (CASCADE), user_profile_id FK (CASCADE), privy_user_id, registration_id FK → tournament_registrations (SET NULL — linked once the slot is taken), payment_method (token/bank/**cash** — cash v2.43.0: user-selected, admin-approved with a note, no comprobante; offered only when `service_payment_methods.cash_enabled` + a COP fee), amount_tokens/amount_cop, wallet_address, tx_hash (lower() UNIQUE partial), block_number, bank_account_id FK, comprobante_url, status (`tournament_entry_status`: pending_bank/confirmed/rejected/cancelled), rejection_reason, reviewed_by/at. Partial UNIQUE: one in-flight (pending_bank/confirmed) order per user+tournament. A `confirmed` order with null registration_id = paid-but-full → manual refund |
| `tournament_prizes` | tournament_id FK → tournaments (CASCADE), position (1–3 unique per tournament), prize_type (tokens/cop/both), amount_tokens (nullable NUMERIC), amount_cop (nullable INTEGER) — DB CHECK enforces type/amount consistency. **reward_text + reward_image_url** (nullable, v2.51.0) — optional per-podium physical reward (e.g. a card set) the winner claims in person; shown under each place on the public podium |
...
| `leagues` | **(v2.56.0)** id PK, tournament_id FK → tournaments (UNIQUE CASCADE), status (league_status enum: draft/in_progress/completed), points_win/points_draw/points_loss (default 3/1/0), tiebreaker_order (text[]: head_to_head/wins/goal_diff/goals_for), participant_count, rounds, created_at, updated_at. Mirror of `brackets` for the round-robin Liga format. Selected when `tournaments.competition_format='league'`. **Standings are DERIVED on read (`src/lib/league/standings.ts`), never stored.** |
| `league_participants` | **(v2.56.0)** id PK, league_id FK → leagues (CASCADE), display_name, seed (1-based), user_profile_id FK → user_profiles (SET NULL), created_at. Mirror of `bracket_participants`. |
| `league_matches` | **(v2.56.0)** id PK, league_id FK → leagues (CASCADE), round, match_number, p1_id/p2_id FK → league_participants (SET NULL), p1_score/p2_score, winner_id FK → league_participants, is_draw (bool), state (reuses match_state enum), created_at, updated_at. Calendar generated by `generateRoundRobin()` (circle method). |
| `payment_events` | **Unified payment ledger (v2.42.0-data)** — one row per payment, polymorphically linked to ANY order via `(order_kind, order_id)` (order_kind enum: pass/tournament_entry/token_purchase/enrollment; **no FK**, like `enrollments.product_type`). method (payment_method enum: token/wire/cash/card), amount_cop **xor** amount_tokens (CHECK), status (payment_event_status enum: pending/confirmed/rejected/cancelled), tx_hash (**GLOBAL** lower() UNIQUE — cross-kind on-chain replay block), block_number, wallet_address, bank_account_id FK, comprobante_url, **recorded_by_admin + reason (CHECK: both required when method='cash')**, idempotency_key, `stripe_payment_intent_id`/`stripe_checkout_session_id` (card, v2.47.0), audit cols. Written **only** by the `apply_payment_event()` RPC. **Live across all four paid services** (tournament entry, academia, $1UP, 1UP Pass) as of the v2.43.0→v2.46.0 cash rollout + the v2.47.0 card path. `wire`↔legacy `'bank'` bridged in `src/lib/payments/methodRegistry.ts` (DB values never renamed) |
| `service_payment_methods` | Per-service enabled-methods config (v2.42.0-data) — service (order_kind PK), token_enabled/wire_enabled (default true), cash_enabled/card_enabled (default false). Admin-editable; card stays user-hidden until env `PAYMENTS_CARD_LIVE` |

> **RPC `apply_payment_event(p_order_kind, p_order_id, p_event_id?, p_method?, p_amount_cop?, p_amount_tokens?, p_tx_hash?, …, p_recorded_by_admin?, p_reason?)`** — the atomic cornerstone of the unified payment layer. Serializes concurrent callers on the same order via a **transaction-scoped advisory lock**, enforces the **v1 single-confirmed invariant** (≤1 confirmed event/order), and returns `{ ok, became_paid, event_id?, reason? }` — `became_paid` is **true for exactly one caller**. Confirms an existing pending event (`p_event_id`) **or** inserts a new confirmed event (token success, cash). **Fulfillment (register_for_tournament / pass activation / course access) fires iff `became_paid`.** Amount-correctness stays with the caller (token = `verifyPassTransfer` exact-match); the RPC owns the race/idempotency guarantee. Deposits (>1 event, SUM math) are deferred — v1 fulfills only on a single full payment. See [`payments-layer.md`](payments-layer.md). |
...
| `POST /api/webhooks/stripe` | Stripe signature | **v2.47.0; all 4 order kinds in v2.52.0** — card / Stripe Checkout. **Inert unless `PAYMENTS_CARD_LIVE` + `STRIPE_WEBHOOK_SECRET` set.** Verifies the signature FIRST (`constructStripeEvent`), then on `checkout.session.completed` + `paid` records the card payment via `apply_payment_event` (idempotent: single-confirmed + `stripe_payment_intent_id` UNIQUE) and `recordAndFulfill` dispatches by `order_kind`: **tournament_entry** (register), **pass** (activate via `computePassWindow`), **enrollment** (`payment_status='approved'`), **token_purchase** (records the COP receipt **only** — the order stays `pending` with an `admin_notes` flag *"pendiente envío de $1UP"*; the admin still sends the tokens on-chain via `approved_tx_hash`). Decision logic is pure in `src/lib/payments/stripeWebhookDecision.ts`. |
...
| `GET\|POST\|PATCH\|DELETE /api/admin/leagues` | isAdmin | **(v2.56.0)** League (round-robin) mirror of `/api/admin/brackets`. POST seeds a DRAFT round-robin calendar from registered/attended participants; PATCH actions `start`/`result` (`{matchId,p1Score,p2Score}`)/`undo`; on completion auto-writes top-3 to `tournament_results` (same manual-override-preserving contract as brackets). GET returns `{league, participants, matches, standings}` (standings computed on read). |
| `GET /api/tournaments/[slug]/standings` | public | **(v2.56.0)** League standings — mirror of `/api/tournaments/[slug]/bracket`. Only `in_progress`/`completed` leagues are public; standings computed on read. Polled by the public page + TV view. |
| `POST\|PATCH /api/admin/payment-events` | isAdmin | Unified payment layer (v2.42.0). POST records a **cash** payment against an order (`{ orderKind, orderId, amountCop?\|amountTokens?, reason }`) via `apply_payment_event` (recorded_by_admin = admin email; mandatory reason) → fulfills iff `became_paid`. PATCH voids a mistaken entry via a compensating `cancelled` event (no UPDATE; fulfillment is **not** auto-reversed — no refunds v1). Ownership/validity in `src/lib/payments/*`. |
| `GET\|PATCH /api/admin/service-payment-methods` | isAdmin | Read/update the per-service enabled-methods matrix (`service_payment_methods`). Card toggle persists but stays user-hidden until `PAYMENTS_CARD_LIVE`. |
