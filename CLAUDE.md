...
| `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` / `NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY` | **Reserved** — unused until `PAYMENTS_CARD_LIVE` is on. The Stripe webhook route is intentionally NOT in the codebase until card goes live (a gated-off webhook is liability for no value). |
