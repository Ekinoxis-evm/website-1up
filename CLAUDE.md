...
| `admin/(protected)/torneos/[slug]/manage` | `admin/` | **Unified tournament cockpit** — one page per tournament with header (status pill, public/TV/QR/edit/cancel/delete), lifecycle banner, and four cards (Info / Participantes / Bracket / Premios y resultados). Cards show inline preview state and deep-link to the standalone admin pages for heavy editing. Edit deep-link uses `/admin/torneos?edit=<id>` (auto-opens the list page's edit modal). |
